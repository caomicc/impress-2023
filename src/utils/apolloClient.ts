...
import { CacheState, ClientProps } from './types';
...
const typePolicies = {
  ...
  Item: {
    fields: {
      appearanceOn: (
        appearance: any,
        { args, readField, toReference }: any,
      ) => {
      ...
      currentUserOwnsThis: (cachedValue: null, { readField }: any) => {
        if (cachedValue != null) {
          return cachedValue;
        }
        ...
        const theyOwnThisItem = itemsTheyOwn.some(
          (itemRef: any) => readField('id', itemRef) === thisItemId,
        );
        return theyOwnThisItem;
      },
      currentUserWantsThis: (cachedValue: null, { readField }: any) => {
        if (cachedValue != null) {
          return cachedValue;
        }
...
const buildAuthLink = (getAuth0: ClientProps) =>
...
async function getAccessToken(getAuth0: any) {
...
const prebuiltCacheState: Record<string, any> = {};
for (const zone of cachedZones) {
  prebuiltCacheState[`Zone:${zone.id}`] = { __typename: 'Zone', ...zone };
}

const buildLink = (getAuth0: ClientProps) =>
  buildAuthLink(getAuth0)
    .concat(authModeLink)
    .concat(
      createPersistedQueryLink({
        generateHash: (query) => sha256(query),
 ...
 */
const buildClient = ({ getAuth0, initialCacheState }: ClientProps) => {
...
function mergeCacheStates(cacheStates: CacheState[]) {
  const mergedCacheState: CacheState = {};
  for (const cacheState of cacheStates) {
    for (const key of Object.keys(cacheState)) {
      if (mergedCacheState[key]) {
        mergedCacheState[key] = {
          ...mergedCacheState[key],
          ...cacheState[key],
        };
      } else {
        mergedCacheState[key] = cacheState[key];
      }
    }
  }
...
function sha256(_query: DocumentNode): string | PromiseLike<string> {
