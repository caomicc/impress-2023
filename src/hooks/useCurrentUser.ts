...
function useCurrentUserViaAuth0({ isEnabled }: { isEnabled: boolean }): User {
...
function useCurrentUserViaDb({ isEnabled }: { isEnabled: boolean }) {
...
function getUserInfoFromAuth0Data(user: any) {
  return {
...
export function getAuthModeFeatureFlag() {
  ...
  let savedValue;
  try {
    savedValue = savedValueString ? JSON.parse(savedValueString) : null;
  } catch (error) {
    console.warn(`DTIAuthModeFeatureFlag was not valid JSON. Ignoring.`);
    savedValue = null;
  }

  if (!['auth0', 'db', null].includes(savedValue as string | null)) {
    console.warn(
      `Unexpected DTIAuthModeFeatureFlag value: %o. Ignoring.`,
      savedValue,
    );
