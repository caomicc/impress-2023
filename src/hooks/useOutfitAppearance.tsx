...
} from '../utils/getVisibleLayers';
import { OutfitState } from '@/utils/types';
 ...
 */
export default function useOutfitAppearance(outfitState: OutfitState) {
  ...
  const itemAppearances = useMemo(
    () => (items || []).map((i: { appearance: any }) => i.appearance),
    [items],
  );
